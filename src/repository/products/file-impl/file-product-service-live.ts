...
export const makeProductFileSystemRepoLive = L.effect(
  ...
  T.gen(function* (_) {
    ...
    const readProductsHashMap = T.gen(function* (_) {
      ...
      const products = yield* _(productsString, Sc.parse(Sc.ParseJson), T.flatMap(Sc.parse(Sc.array(Product))), T.tapError(T.logError))

      return pipe(products, A.map(product => product.id !== undefined ? O.some([product.id, product] as const) : O.none()), A.compact, HM.fromIterable)

    })

    const writeProductsHashMap = (products: HM.HashMap<number & Brand<"ProductId"> | undefined, Product>) => fs.writeFileString(PATH, pipe(products, HM.values, A.fromIterable, JSON.stringify))



    const getOneProductRepo: ProductRepositoryService['getOneProductRepo'] = id =>
      pipe(readProductsHashMap, T.flatMap(HM.get(id)), T.mapError(_ => ProductNotFoundError.of('Product not found')))
    ...
    const patchOneProductRepo: ProductRepositoryService['patchOneProductRepo'] = product => T.gen(function* (_) {
      if (product.id === undefined) return yield* _(ProductNotFoundError.of('Product id is undefined'))
      const newProducts = yield* _(pipe(readProductsHashMap, T.map(HM.modify(product.id, _ => product))))
    ...
    const removeOneProductRepo: ProductRepositoryService['removeOneProductRepo'] = productId =>
      pipe(readProductsHashMap, T.map(HM.remove(productId)), T.flatMap(writeProductsHashMap), T.zipRight(T.succeed(productId)))
